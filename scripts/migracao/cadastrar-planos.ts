 ...
 * e ativar. A duração vem da moda dos contratos reais (Início → Final) dos
...
function mesesDeContrato(inicioISO: string | null, fimISO: string | null): number | null {
...
async function main() {
  ...
  for (const a of alunos) {
    ...
    g.alunos++;
    const meses = mesesDeContrato(a.inicioISO, a.fimISO);
    if (meses !== null) g.duracoes.push(meses);
  ...
  for (const [chave, g] of [...porPlano.entries()].sort((a, b) => b[1].alunos - a[1].alunos)) {
    const duracao = g.duracoes.length ? moda(g.duracoes) : 1;
    if (jaTem.has(chave)) {
      pulados++;
    ...
    criados++;
    linhas.push(`  + ${g.nome}  — ${g.alunos} alunos, ${duracao} meses`);
    if (apply) {
      await prisma.plan.create({
        data: {
          unitId: unit.id, nome: g.nome, valorMensal: 0, duracaoMeses: duracao, ativo: false,
